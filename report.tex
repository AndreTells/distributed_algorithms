...
Exclusão mutua
Exclusão mutua não trata casos de vários threads, necessitaria de várias
conexões em paralelo na implementação atual. 

As threads leitoras e a thread para responder pedidos, por sua vez, na
implementação atual, poderiam ser unidas em uma única thread. Porém, afim de
manter um nível de separação entre a lógica das mensagens, foi decidido manter
as duas separadas.

Nenhum limite de clientes é explicitamente imposto, que em uma aplicação real
poderia ser um vetor de ataque ao sistema.

Eleição
TCP é ruim para esta situação
